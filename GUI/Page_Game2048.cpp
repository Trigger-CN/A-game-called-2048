...
static void NewRowBtnCreate()
{
    ...
    int numBtnIndex = 0;
    int numLabelVal = 0;
    for (numOfBtn; numOfBtn > 0; numOfBtn--)
    {
...
static void Event(lv_obj_t* obj, lv_event_t event)
{
    ...
    {
        uint8_t GESTURE_DIR = lv_indev_get_gesture_dir(lv_indev_get_act());
        if (GESTURE_DIR == LV_GESTURE_DIR_TOP)
        {
            AllShift(&btnMatrix, SD_Up);
        }
        else if (GESTURE_DIR == LV_GESTURE_DIR_RIGHT)
        {
            AllShift(&btnMatrix, SD_Right);
        }
        else if (GESTURE_DIR == LV_GESTURE_DIR_LEFT)
        {
            AllShift(&btnMatrix, SD_Left);
        }
        else if (GESTURE_DIR == LV_GESTURE_DIR_BOTTOM)
        {
...
{
    GamePrepare();
    lv_obj_set_event_cb(lv_scr_act(),Event);
    ContBtnGrp_Create(lv_scr_act());
